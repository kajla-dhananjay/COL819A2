...
void DFS_Util(int N, std::set<int> &edge_weights, std::vector<std::tuple<int, int, int> > &edges, std::vector<std::set<int> > &adj_list, std::vector<int> &color, std::unordered_map<int, int> &colormap)
{
  ...
  for(int i = 1; i < color_cnt; i++)
  {
    ...
    int r = distw(generator); // random weight
